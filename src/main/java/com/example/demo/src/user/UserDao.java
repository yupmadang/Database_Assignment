...
public class UserDao {
    ...
    public int createUser(PostUserReq postUserReq) {
        String createUserQuery = "insert into User (name, Id, temp, region_idx) VALUES (?,?,?,?)"; // 실행될 동적 쿼리문
        Object[] createUserParams = new Object[]{postUserReq.getName(), postUserReq.getId(), postUserReq.getTemp(),postUserReq.getRegion_idx()}; // 동적 쿼리의 ?부분에 주입될 값
    ...
    public int checkId(String name) {
        String checkIdQuery = "select exists(select name from User where name = ?)"; // User Table에 해당 email 값을 갖는 유저 정보가 존재하는가?
        String checkIdParams = name; // 해당(확인할) 아이디 값
    ...
    public int modifyUserName(PatchUserReq patchUserReq) {
        String modifyUserNameQuery = "update User set name = ? where idx = ? "; // 해당 userIdx를 만족하는 User를 해당 name으로 변경한다.
        Object[] modifyUserNameParams = new Object[]{patchUserReq.getName(),patchUserReq.getIdx()}; // 주입될 값들(nickname, userIdx) 순
    ...
    public User getId(PostLoginReq postLoginReq) {
        String getIdQuery = "select idx, Id, name,temp, region_idx from User where Id = ?"; // 해당 email을 만족하는 User의 정보들을 조회한다.
        String getIdParams = postLoginReq.getId(); // 주입될 email값을 클라이언트의 요청에서 주어진 정보를 통해 가져온다.
    ...
    public List<GetUserRes> getUsers() {
        ...
        return this.jdbcTemplate.query(getUsersQuery,
                (rs, rowNum) -> new GetUserRes(
                        ...
                        rs.getString("Id"),
                        rs.getString("temp"),
                        rs.getInt("region_idx")) // RowMapper(위의 링크 참조): 원하는 결과값 형태로 받기
